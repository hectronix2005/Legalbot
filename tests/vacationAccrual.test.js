...
} = require('../backend/services/vacationAccrualService');
...
describe('calculateAccruedVacationDays() - Casos Básicos', () => {
  ...
  test('Caso 2: Exactamente 1 año regular retorna 15 días', () => {
    ...
    expect(result.accruedDays).toBeCloseTo(15, 2);
  ...
  test('Caso 4: 5 años retorna 75 días', () => {
    ...
    // ~5 años = ~75 días (con ajuste por bisiestos)
    expect(result.accruedDays).toBeGreaterThan(74.9);
    expect(result.accruedDays).toBeLessThan(75.1);
  });
...
describe('calculateAccruedVacationDays() - Validaciones de Error', () => {
  ...
  test('Fecha de contratación inválida retorna error', () => {
    const result = calculateAccruedVacationDays(null, new Date());

    expect(result.accruedDays).toBe(0);
    expect(result.error).toBeTruthy();
    expect(result.error).toContain('fecha válida');
  });
...
describe('calculateAccruedVacationDays() - Casos Borde', () => {
  ...
  test('Contratación 29 de febrero año bisiesto', () => {
    ...
    // ~365 días ≈ 15 días
    expect(result.accruedDays).toBeCloseTo(15, 1);
    expect(result.error).toBeNull();
  });
...
describe('calculateAccruedVacationDays() - Años Bisiestos', () => {
  ...
  test('Año bisiesto completo (2024) causa 15 días', () => {
    ...
    // 365 días en 2024 (día 31 no cuenta como día completo)
    expect(result.daysWorked).toBe(365);
    expect(result.accruedDays).toBeCloseTo(14.9589, 2); // 365 × 15/366
  });
...
describe('getVacationAccrualDetail()', () => {
  ...
  test('Detalle completo de causación', () => {
    ...
    expect(result.detail.accruedDays).toBeCloseTo(15, 2);
    expect(result.detail.availableDays).toBeCloseTo(15, 2);
  ...
  test('Error propagado correctamente', () => {
    const result = getVacationAccrualDetail(null, 0, 0, new Date());
